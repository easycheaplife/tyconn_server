...
function M.claim_mail_items(user_id, mail_id)
    ...
    if mail.items and #mail.items > 0 then
        ...
        local ok, err = item_service.add_items_to_slot_new(user_id, mail.items, enum.ChangeSource.SOURCE_MAIL)
        if not ok then
...
function M.batch_claim_items(user_id, mail_ids)
    ...
    if #items_to_add > 0 then
        -- 添加物品到背包
        local item_service = require "services.item_service"
        local ok, err = item_service.add_items_to_slot_new(user_id, items_to_add, enum.ChangeSource.SOURCE_MAIL)
        if not ok then
            return false, err
        end
